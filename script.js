...
function displayQuestions(data, subject, cls) {
  ...
  // Check if basic version - limit to 5 questions
  const isBasicVersion = localStorage.getItem("examVersion") === "basic";
  const questionLimit = isBasicVersion ? 10 : 20;

  // Limit to questionLimit questions or less if there aren't enough questions
  ...
  questionsDiv.innerHTML = `
      ...
      ${
        isBasicVersion
          ? '<div class="basic-version-badge">Free Version - Limited to 10 questions</div>'
          : ""
      }
  ...
  // Add submit button and upgrade banner for basic version
  if (isBasicVersion) {
    questionsDiv.innerHTML += `
      <div class="upgrade-banner-exam">
...
function checkAnswers() {
  ...
  const resultData = {
    ...
    responses: responses,
    version: localStorage.getItem("examVersion") || "pro",
  };

  // For basic version, we'll skip saving to Google Sheets
  const isBasicVersion = localStorage.getItem("examVersion") === "basic";
  ...
  if (isBasicVersion) {
    ...
    // Save to appropriate sheet (Pro version)
...
function displayResults(resultData) {
  ...
  // Basic version results with upgrade prompt
  if (isBasicVersion) {
    questionsDiv.innerHTML = `
      <div class="results-container">
        <div class="basic-version-badge">Basic Version Results</div>
        ...
        <div class="results-summary">
          ...
          <div class="score-display">
            <h3 class="Grade"><span style="color: ${gradeColor}; text-shadow:${gradeColor} 2px 2px 5px; font-weight: bold;">${grade}</span></h3>
...
// Timer functionality
// Reduced time for basic version (15 minutes instead of 30)
const examDuration =
  localStorage.getItem("examVersion") === "basic" ? 15 * 60 : 30 * 60;
let timerInterval;
